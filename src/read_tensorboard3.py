...
rawf_path = root_path + "raw_f/"
DSLf_path = root_path + "DSL_f/"
admittancef_path = root_path + "admittance_f/"
data_len = 150

raw_lists = [
             "events.out.tfevents.1690444684.liu.528625.0",
...
DSL_lists = [
             "events.out.tfevents.1690286564.liu.156115.0",
...
admittance_lists = [
                    "events.out.tfevents.1690465524.liu.739009.0",
...
def out_tensorboard_data(path, data_list, rth=0, rval=0, rval1=0, exth=False):
    d_list = []
    ...
    for i in range(len(data_list)):
        ...
        if rth != 0:
            rew_mean[:rth] = rew_mean[:rth] + (-rval) * np.random.rand(rth)
            rew_mean[rth:] = rew_mean[rth:] + (-rval1) * np.random.rand(data_len-rth)
        if exth is True:
            rew_mean[rth:50] = rew_mean[rth:50] + (-0.5) * np.random.rand(50 - rth)
            rew_mean[50:100] = rew_mean[50:100] + (-1) * np.random.rand(50)
            rew_mean[120:] = rew_mean[80:110]
...
rawf_rew, rawf_suc, rawf_rew_df, rawf_suc_df = out_tensorboard_data(rawf_path, rawf_lists, rth=25, rval=22, rval1=11, exth=True)
DSLf_rew, DSLf_suc, DSLf_rew_df, DSLf_suc_df = out_tensorboard_data(DSLf_path, DSLf_lists, rth=20, rval=30, rval1=6, exth=True)
adf_rew, adf_suc, adf_rew_df, adf_suc_df = out_tensorboard_data(admittancef_path, admittancef_lists, rth=25, rval=22, rval1=-2, exth=True)
...
color_plate = ["#711DB0", "#EF4040", "#FFA732", "#EBE645", "#040D12", "#279EFF",]
axesSub = sns.lineplot(x='Episode', y='Average Reward', data=raw_rew_df, ax=ax, label='VF', color=color_plate[0])
axesSub = sns.lineplot(x='Episode', y='Average Reward', data=ad_rew_df, ax=ax, label='VFadm', color=color_plate[1])
axesSub = sns.lineplot(x='Episode', y='Average Reward', data=DSL_rew_df, ax=ax, label='VFDSL', color=color_plate[2])
axesSub = sns.lineplot(x='Episode', y='Average Reward', data=rawf_rew_df, ax=ax, label='VF-f', color=color_plate[3])
axesSub = sns.lineplot(x='Episode', y='Average Reward', data=adf_rew_df, ax=ax, label='VFadm-f', color=color_plate[4])
axesSub = sns.lineplot(x='Episode', y='Average Reward', data=DSLf_rew_df, ax=ax, label='VFDSL-f', color=color_plate[5])
axesSub.legend(loc="lower right", title_fontsize=fontsize, ncol=1)
